...
IF ~Global("ZB_STRIV_REWARD_3","GLOBAL",1)~ THEN BEGIN 0
  SAY @3051 /* Off with you, then. I got work to do. Daddy's coming, darlings! */
  IF ~~ THEN EXIT
END

IF ~Global("ZB_STRIV_TRAINING_CHECK","GLOBAL",1) Global("ZB_STRIV_REWARD_1","GLOBAL",0)~ THEN BEGIN 1
...
IF ~Global("OHB_FINALE","GLOBAL",0) Global("ZB_STRIV_REWARD_3","GLOBAL",0) Global("ZB_STRIV_HELLO","GLOBAL", 0)~
THEN BEGIN 3
  SAY @3038 /* Look at my puppies. Pretty, aren't they? You wanna pet 'em? Take one for a walk? */
  IF ~~ THEN DO ~SetGlobal("ZB_STRIV_HELLO","GLOBAL",1)~ GOTO 4
END
...
IF ~Global("ZB_STRIV_HELLO","GLOBAL",1)~ THEN BEGIN 6
  ...
  IF ~~ THEN REPLY @973 /* Not right now. */ GOTO 10
END
